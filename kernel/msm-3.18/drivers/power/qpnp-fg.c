...
// khs add battery type. 
#define USE_BATTERY_TYPE
#define USE_5_AND_95_PERCENT
#define USE_BATT_CAPACITY_MAH
#endif
...
#if defined(USE_BATT_CAPACITY_MAH)
	int64_t			last_current_capacity;
	bool			first_input_present;
	bool			first_read;
#endif
	int			init_cc_pc_val;
	bool			active;
...
static int get_battery_type(char * batterytype, int len)
{
	return _get_cmdline_param("androidboot.battery_type=", batterytype, len);
}
#endif
...
#if defined(USE_5_AND_95_PERCENT)
	int msoc_temp, mcapacity;
#endif
...
#if defined(USE_5_AND_95_PERCENT)
	msoc_temp = DIV_ROUND_CLOSEST((msoc - 1) * (FULL_CAPACITY - 2), FULL_SOC_RAW - 2) + 1;
	mcapacity = DIV_ROUND_CLOSEST((msoc_temp - 5) * 100, 90);
	
	if (mcapacity <= 0) {
		return EMPTY_CAPACITY;
	}
	else if (mcapacity >= 100) {
		return FULL_CAPACITY;
	}
	else  {
		return mcapacity;
...
#if defined(USE_BATT_CAPACITY_MAH)
	if (!chip->learning_data.first_read) {
		chip->learning_data.first_input_present = is_input_present(chip);
		chip->learning_data.first_read = 1;
	}

	if (!chip->learning_data.first_input_present) {
		capacity = get_prop_capacity(chip);
		cc_soc = cc_soc + ((100 - capacity) * CAPACITY_COUNT);
	}
#endif
	current_capacity = cc_soc * chip->learning_data.learned_cc_uah;
	do_div(current_capacity, FULL_PERCENT_28BIT);
#if defined(USE_BATT_CAPACITY_MAH)
	if (chip->learning_data.last_current_capacity <= 0) {
		chip->learning_data.last_current_capacity = chip->learning_data.learned_cc_uah;
	}
	if (current_capacity > chip->learning_data.learned_cc_uah) {
		capacity_temp = current_capacity - chip->learning_data.learned_cc_uah;
		if (capacity_temp > 100000) {
			current_capacity = chip->learning_data.last_current_capacity;
		}
		chip->learning_data.last_current_capacity = current_capacity;
		chip->learning_data.first_input_present = 0;
	}
#endif
...
#if defined(USE_BATT_CAPACITY_MAH)
static void fg_cap_learning_load_data(struct fg_chip *chip, uint32_t capacity_mah)
#else
...
static int fg_batt_profile_init(struct fg_chip *chip)
{
	...
	char battery_dts[64] = {0,};
	unsigned long value;
	int max_battery_typ;
#endif
#if defined(USE_BATT_CAPACITY_MAH)
	uint32_t m_capacity_mah;
#endif
...
#if defined(USE_BATTERY_TYPE)
	ret = get_battery_type(battery_type, sizeof(battery_type) - 1);
	if(ret == 0){
		if (kstrtoul(battery_type, 0, &value)){
			pr_err("Invalid battery_type :%s\n", battery_type);
		}

		ret = of_property_read_u32(profile_node, "markt,max-battery-type",
				&max_battery_typ);
		if (ret) {
			pr_err("%s: failed to read max_battery_typ\n", __func__);
		}

		if (value >= max_battery_typ) {
			strcpy(battery_type, "0");
			pr_err("battery_type is default 0 !! \n");
		}
	} else {
		pr_err("get_battery_type() failed\n");
		strcpy(battery_type, "0");
		pr_err("battery_type is default 0 !! \n");
	}
#endif
...
#if defined(USE_BATTERY_TYPE)
	strcpy(battery_dts, "qcom,chg-rs-to-rslow_");
	strcat(battery_dts, battery_type);
	rc = of_property_read_u32(profile_node, battery_dts,
...
#if defined(USE_BATTERY_TYPE)
	strcpy(battery_dts, "qcom,chg-rslow-comp-c1_");
	strcat(battery_dts, battery_type);
	rc = of_property_read_u32(profile_node, battery_dts,
...
#if defined(USE_BATTERY_TYPE)
	strcpy(battery_dts, "qcom,chg-rslow-comp-c2_");
	strcat(battery_dts, battery_type);
	rc = of_property_read_u32(profile_node, battery_dts,
...
#if defined(USE_BATTERY_TYPE)
	strcpy(battery_dts, "qcom,chg-rslow-comp-thr_");
	strcat(battery_dts, battery_type);
	rc = of_property_read_u32(profile_node, battery_dts,
...
#if defined(USE_BATTERY_TYPE)
	strcpy(battery_dts, "qcom,max-voltage-uv_");
	strcat(battery_dts, battery_type);
	rc = of_property_read_u32(profile_node, battery_dts,
...
#if defined(USE_BATTERY_TYPE)
	strcpy(battery_dts, "qcom,fg-profile-data_");
	strcat(battery_dts, battery_type);
	data = of_get_property(profile_node, battery_dts, &len);
#else
...
#if defined(USE_BATTERY_TYPE)
	strcpy(battery_dts, "qcom,battery-type_");
	strcat(battery_dts, battery_type);
	rc = of_property_read_string(profile_node, battery_dts,
					&batt_type_str);
#else
...
#if defined(USE_BATT_CAPACITY_MAH)
		strcpy(battery_dts, "qcom,nom-batt-capacity-mah_");
		strcat(battery_dts, battery_type);
		of_property_read_u32(profile_node, battery_dts, &m_capacity_mah);
		fg_cap_learning_load_data(chip, m_capacity_mah);
#else
...
#if defined(USE_BATT_CAPACITY_MAH)
	chip->learning_data.first_read = 0;
#endif
