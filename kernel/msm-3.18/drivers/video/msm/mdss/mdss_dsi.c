...
// khs add lcd type. 
#define USE_LCD_TYPE
#define NT51018C_DESK 0
#define ILI7807D_FACE 1
unsigned long g_lcd_type_value = 0;
char g_lcd_type[3] = {0,};
#endif

static struct pm_qos_request mdss_dsi_pm_qos_request;

#if defined(USE_LCD_TYPE)
// khs add lcd type.
...
static int get_lcd_type(char * lcdtype, int len)
{
	return _get_cmdline_param("androidboot.lcd_type=", lcdtype, len);
}
#endif
		...
		struct platform_device *pdev)
{
	...
	pr_debug("%s:%d: Select primary panel from dt\n",
					__func__, __LINE__);
#if defined(USE_LCD_TYPE)
	strcpy(lcd_dts, "qcom,dsi-pref-prim-pan_");
	strcat(lcd_dts, g_lcd_type);
...
#if defined(USE_LCD_TYPE)
		strcpy(lcd_dts, "qcom,platform-enable-gpio_");
		strcat(lcd_dts, g_lcd_type);
...
#if defined(USE_LCD_TYPE)
		strcpy(lcd_dts, "qcom,platform-te-gpio_");
		strcat(lcd_dts, g_lcd_type);
...
#if defined(USE_LCD_TYPE)
	strcpy(lcd_dts, "qcom,platform-bklight-en-gpio_");
	strcat(lcd_dts, g_lcd_type);
...
#if defined(USE_LCD_TYPE)
	strcpy(lcd_dts, "qcom,platform-reset-gpio_");
	strcat(lcd_dts, g_lcd_type);
...
#if defined(USE_LCD_TYPE)
    ret = get_lcd_type(g_lcd_type, sizeof(g_lcd_type) - 1);
    if(ret == 0){
        if (kstrtoul(g_lcd_type, 0, &g_lcd_type_value)){
            pr_err("Invalid lcd_type :%s\n", g_lcd_type);
        }
    } else {
        pr_err("get_lcd_type() failed\n");
        strcpy(g_lcd_type, "0");
        g_lcd_type_value = 0;
        pr_err("lcd_type is default 0 !! \n");
    }

    if (g_lcd_type_value == NT51018C_DESK) {
        pr_err("mdss_dsi_ctrl_driver_init lcd type = %lu, NT51018C_DESK \n", g_lcd_type_value);
    }
	else if (g_lcd_type_value == ILI7807D_FACE) {
        pr_err("mdss_dsi_ctrl_driver_init lcd type = %lu, ILI7807D_FACE \n", g_lcd_type_value);
    }
#endif
