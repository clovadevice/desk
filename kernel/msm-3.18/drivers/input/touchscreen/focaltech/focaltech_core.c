...
unsigned long g_touch_type_value = 0;
char g_touch_type[3] = {0,};
#endif
...
static int get_touch_type(char * touchtype, int len)
{
	return _get_cmdline_param("androidboot.touch_type=", touchtype, len);
}
#endif
...
/*		temp = event->au16_x[i];
		...
		event->au16_y[i] = data->pdata->x_max - temp;*/
...
#if defined(USE_TOUCH_TYPE)
	strcpy(touch_dts, "focaltech,display-coords_");
	strcat(touch_dts, g_touch_type);
    if (!strcmp(name, touch_dts))
#else
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
	...
	strcpy(touch_dts, "focaltech,display-coords_");
	strcat(touch_dts, g_touch_type);
	rc = fts_get_dt_coords(dev, touch_dts, pdata);
#else
...
#if defined(USE_TOUCH_TYPE)
	strcpy(touch_dts, "focaltech,num-max-touches_");
	strcat(touch_dts, g_touch_type);
	rc = of_property_read_u32(np, touch_dts, &temp_val);
#else
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	for(i=0;i<FTS_MAX_POINTS;i++){
		data->event.pressure[i] = FTS_TOUCH_UP;
	}
...
static int __init fts_ts_init(void)
{
    ...
    FTS_FUNC_ENTER();
#if defined(USE_TOUCH_TYPE)
    ret = get_touch_type(g_touch_type, sizeof(g_touch_type) - 1);
    if(ret == 0){
        if (kstrtoul(g_touch_type, 0, &g_touch_type_value)){
            pr_err("Invalid touch_type :%s\n", g_touch_type);
        }
    } else {
        pr_err("get_touch_type() failed\n");
        strcpy(g_touch_type, "0");
        g_touch_type_value = 0;
        pr_err("touch_type is default 0 !! \n");
    }

    if (g_touch_type_value == FT5626_DESK) {
        pr_err("fts_ts_init touch type = %lu, FT5626_DESK \n", g_touch_type_value);
    }
	else if (g_touch_type_value == FT5446_FACE) {
        pr_err("fts_ts_init touch type = %lu, FT5446_FACE \n", g_touch_type_value);
		chip_types = chip_types_1;
		fts_updatefun.chip_idh = FTS_CHIP_IDH_1;
    }
