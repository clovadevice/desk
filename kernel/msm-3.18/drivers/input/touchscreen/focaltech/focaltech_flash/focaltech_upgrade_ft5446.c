...
#if defined(CONFIG_TARGET_PRODUCT_IF_S700N) || defined(USE_TOUCH_TYPE)
#define FWVER_INDEX					0x10A
#endif
/*****************************************************************************
...
static int fts_ctpm_get_vendor_id(struct i2c_client * client)
{
...
#else
    vendorid_1 = FTS_VENDOR_1_ID;
    vendorid_2 = FTS_VENDOR_2_ID;
#endif
    ...
    if (fts_updatefun_curr.app_valid == false)
    {
        ...
        if ((vendorid[1] != vendorid_1) && (vendorid[1] != vendorid_2))
        {
            fts_updatefun_curr.need_upgrade = false;
            FTS_ERROR( "[UPGRADE]: vendor id error : 0x%x, can not upgrade!!", vendorid[1]);
            return -EIO;
        }
        ...
        FTS_DEBUG("[UPGRADE]: vendor id : 0x%x!!", vendorid[1]);
    }
    return 0;
}
...
static int fts_ctpm_fw_upgrade_use_buf(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
    ...
    if (i_ret < 0)
    {
        FTS_ERROR( "[UPGRADE]: read project id in flash fail!!");
        return i_ret;
    }

    if (!fts_updatefun_curr.need_upgrade)
    {
        FTS_DEBUG( "[UPGRADE]: no need upgrade!!");
        FTS_DEBUG("[UPGRADE]: reset the new TP!!");
...
static int fts_ctpm_fw_upgrade_with_app_bin_file(struct i2c_client *client, char *firmware_name)
{
    ...
    if ((pbt_buf[fwsize-1] != vendorid_1) && (pbt_buf[fwsize-1] != vendorid_2))
    {
