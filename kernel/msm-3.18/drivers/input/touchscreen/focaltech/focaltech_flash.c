...
#if defined(USE_TOUCH_TYPE)
struct ft_chip_t chip_types_1 = FTS_CHIP_TYPE_MAPPING_1;
#endif
...
#if defined(USE_TOUCH_TYPE)
unsigned char CTPM_FW_0[] =
{
#include FTS_UPGRADE_FW_APP_0
};
unsigned char CTPM_FW_1[] =
{
#include FTS_UPGRADE_FW_APP_1
};
#else
...
int fts_ctpm_fw_upgrade_ReadVendorID(struct i2c_client *client, u8 *ucPVendorID)
{
    ...
    *ucPVendorID = 0;
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++)
    {
        ...
        if (reg_val[0] == chip_types.rom_idh && reg_val[1] == chip_types.rom_idl)
        {
...
int fts_ctpm_fw_upgrade_ReadProjectCode(struct i2c_client *client, char *pProjectCode)
{
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++)
    {
        ...
        reg_val[0] = reg_val[1] = 0x00;
        fts_i2c_read(client, auc_i2c_write_buf, 4, reg_val, 2);
        if (reg_val[0] == chip_types.pramboot_idh&& reg_val[1] == chip_types.pramboot_idl)
        {
            FTS_DEBUG("[FTS] Step 3: READ OK CTPM ID,ID1 = 0x%x,ID2 = 0x%x!!", reg_val[0], reg_val[1]);
            break;
        }
...
void fts_ctpm_get_upgrade_array(void)
{
    ...
    {
        if (chip_id==fts_updateinfo[i].CHIP_ID || chip_types.chip_idh==fts_updateinfo[i].CHIP_ID)
        {
    ...
    if ((chip_types.chip_idh == fts_updatefun.chip_idh) && (chip_types.chip_idl == fts_updatefun.chip_idl))
    {
...
#if defined(USE_TOUCH_TYPE)
    if (g_touch_type_value == FT5626_DESK) {
        chip_type = FTS_CHIP_TYPE_0;
    }
	else if (g_touch_type_value == FT5446_FACE) {
        chip_type = FTS_CHIP_TYPE_1;
    }
#else
    chip_type = FTS_CHIP_TYPE;
#endif

    if ((chip_type == _FT8716) || (chip_type == _FT8736) || (chip_type == _FT8607)) {
        if (fw_type == FW_SIZE)
        {
#if defined(USE_TOUCH_TYPE)
            if (g_touch_type_value == FT5626_DESK) {
                fw_len = sizeof(CTPM_FW_0);
            }
	        else if (g_touch_type_value == FT5446_FACE) {
...
unsigned char fts_ctpm_get_pram_or_rom_id(struct i2c_client *client)
{
    ...
    else if (reg_val[0] == chip_types.bootloader_idh && reg_val[1] == chip_types.bootloader_idl)
    {
        ...
        FTS_DEBUG("[UPGRADE]: read bootloader id for success , bootloader id is: ID1 = 0x%x,ID2 = 0x%x!!", reg_val[0], reg_val[1]);
    }
    else
    {
...
u8 fts_ctpm_check_fw_status(struct i2c_client *client)
{
    ...
    u8 i = 0;
#if defined(USE_TOUCH_TYPE)
    u8 vendorid_1, vendorid_1_2, vendorid_1_3, vendorid_2 = 0;
    if (g_touch_type_value == FT5626_DESK) {
        vendorid_1 = FTS_0_VENDOR_1_ID;
        vendorid_1_2 = FTS_0_VENDOR_1_ID_2;
        vendorid_1_3 = FTS_0_VENDOR_1_ID_3;
        vendorid_2 = FTS_0_VENDOR_2_ID;
    }
	else if (g_touch_type_value == FT5446_FACE) {
        vendorid_1 = FTS_1_VENDOR_1_ID;
        vendorid_2 = FTS_1_VENDOR_2_ID;
    }
#endif
    ...
    for (i=0; i<5; i++)
    {
...
#if defined(CONFIG_TARGET_PRODUCT_IF_S700N)
            ((uc_tp_vendor_id == FTS_VENDOR_1_ID) || (uc_tp_vendor_id == FTS_VENDOR_1_ID_2) || (uc_tp_vendor_id == FTS_VENDOR_1_ID_3) || (uc_tp_vendor_id == FTS_VENDOR_2_ID)))
#elif defined(USE_TOUCH_TYPE)
            ((uc_tp_vendor_id == vendorid_1) || (uc_tp_vendor_id == vendorid_1_2) || (uc_tp_vendor_id == vendorid_1_3) || (uc_tp_vendor_id == vendorid_2)))
#else
        ...
        else if (uc_tp_vendor_id == 0xEF && uc_chip_id == 0xEF) {
            fw_status = FTS_RUN_IN_APP;
            FTS_INFO("[UPGRADE]: APP OK!! Touch IC Upgrade");
            break;
        }
        else
        {
...
#if defined(USE_TOUCH_TYPE)
    FTS_DEBUG("[UPGRADE]: uc_tp_vendor_id = %x, uc_chip_id = %x, chip_types.chip_idh = %x, FTS_VENDOR_1_ID = %x!!", uc_tp_vendor_id, uc_chip_id, chip_types.chip_idh, vendorid_1);
#else
    FTS_DEBUG("[UPGRADE]: uc_tp_vendor_id = %x, uc_chip_id = %x, chip_types.chip_idh = %x, FTS_VENDOR_1_ID = %x!!", uc_tp_vendor_id, uc_chip_id, chip_types.chip_idh, FTS_VENDOR_1_ID);
#endif
...
bool fts_ctpm_check_need_upgrade(struct i2c_client *client)
{
    ...
    fts_updatefun_curr.get_projectid = FTS_GET_PROJECT_ID_FROM_FLASH;
    ...
    if (fw_status == FTS_RUN_IN_APP) //call fts_flash_get_upgrade_info in probe function firstly.
    {
        ...
        else if (uc_tp_fm_ver == 0xEF) {
            msleep(300);
            fts_updatefun_curr.need_upgrade = true;
            bUpgradeFlag = true;
            FTS_INFO("[UPGRADE]: need upgrade!! Touch IC Upgrade");
        }
        else
        {
           	msleep(300);
            fts_updatefun_curr.need_upgrade = false;
            FTS_INFO("[UPGRADE]: no need upgrade!!");
        }

        if (fts_updatefun_curr.get_projectid)
        {
            bUpgradeFlag = true;
            FTS_INFO("[UPGRADE]: need get project id!!");
        }
    }
    else
    {
        /* if app is invalid, reset to run ROM */
        FTS_INFO("[UPGRADE]: app is invalid, need upgrade, reset now!!");
...
{
    int i_ret = 0;
    i_ret = fts_updatefun_curr.upgrade_with_app_bin_file(client, FTS_UPGRADE_FW_APP_BIN);
    if (i_ret < 0)
    {
...
{
    int i_ret = 0;
    i_ret = fts_updatefun_curr.upgrade_with_lcd_cfg_bin_file(client, FTS_UPGRADE_LCD_CFG_BIN);
    if (i_ret < 0)
    {
