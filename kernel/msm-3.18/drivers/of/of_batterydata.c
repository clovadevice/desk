...
int get_battery_type(char * batterytype, int len)
{
	return _get_cmdline_param("androidboot.battery_type=", batterytype, len);
}

void set_battery_type(const struct device_node *dev_node)
{
	unsigned long value;
	int max_battery_typ, ret = 0;

	ret = get_battery_type(g_battery_type, sizeof(g_battery_type) - 1);
	if(ret == 0){
		if (kstrtoul(g_battery_type, 0, &value)){
			pr_err("Invalid g_battery_type :%s\n", g_battery_type);
		}

		ret = of_property_read_u32(dev_node, "markt,max-battery-type",
				&max_battery_typ);
		if (ret) {
		...
		if (value >= max_battery_typ) {
			strcpy(g_battery_type, "0");
			pr_err("g_battery_type is default 0 !! \n");
		}
	} else {
...
#if defined(USE_BATTERY_TYPE)
	strcpy(battery_dts, "qcom,batt-id-kohm_");
	strcat(battery_dts, g_battery_type);
	prop = of_find_property(np, battery_dts, NULL);
#else
		...
		const char *psy_name,  const char  *batt_type)
{
...
#if defined(USE_BATTERY_TYPE)
		set_battery_type(node);
#endif
		if (batt_type != NULL) {
#if defined(USE_BATTERY_TYPE)
			strcpy(battery_dts, "qcom,battery-type_");
			strcat(battery_dts, g_battery_type);
			rc = of_property_read_string(node, battery_dts,
							&battery_type);
#else
			rc = of_property_read_string(node, "qcom,battery-type",
							&battery_type);
	...
	strcpy(battery_dts, "qcom,battery-type_");
	strcat(battery_dts, g_battery_type);
	rc = of_property_read_string(best_node, battery_dts,
							&battery_type);
#else
