...
#define QUAT_MI2S2_SLAVE
#define GPIO_EXTERNAL_I2S
#define UNUSED_MSM8X16_WCD
#define MSM8952_SND_CARD_TAS5782
#define DEFAULT_MCLK_RATE 12288000
//#define MCLK
#else
#define DEFAULT_MCLK_RATE 9600000
#endif /* CONFIG_TARGET_PRODUCT_IF_S200N */

#if defined(CONFIG_TARGET_PRODUCT_IF_S700N) || defined(CONFIG_TARGET_PRODUCT_IF_MKT200)
#ifndef MI2S_CLOCK_ALWAYS_ON
#define QUIN_MI2S_SLAVE
#endif /* MI2S_CLOCK_ALWAYS_ON */ 
#endif /* CONFIG_TARGET_PRODUCT_IF_S700N */
...
#if defined(CONFIG_TARGET_PRODUCT_IF_S200N) || defined(CONFIG_TARGET_PRODUCT_IF_S700N) || defined(CONFIG_TARGET_PRODUCT_IF_MKT200)
static int msm_ter_mi2s_tx_ch = 2;
#else
static int msm_ter_mi2s_tx_ch = 1;
#endif
#if defined(CONFIG_TARGET_PRODUCT_IF_S200N)
static int msm_pri_mi2s_rx_ch = 1;
#elif defined(CONFIG_TARGET_PRODUCT_IF_S700N) || defined(CONFIG_TARGET_PRODUCT_IF_MKT200)
...
#ifdef QUAT_MI2S2_SLAVE 
#define LPASS_CSR_GP_IO_MUX_MIC_EXT_CLK_CTL 0xc054000 
void __iomem *quat_ext_clk_addr; 
#endif 

#ifdef QUIN_MI2S_SLAVE
#define LPASS_CSR_GP_IO_MUX_MIC_CTL                     0xc051000
#define LPASS_CSR_GP_IO_MUX_QUI_CTL                     0xc052000
#define LPASS_CSR_GP_LPAIF_QUI_PCM_SEC_MODE_MUXSEL      0xc056000
#define LPASS_CSR_GP_IO_MUX_QUI_EXT_CLK_CTL             0xc054008
void __iomem *quin_gp_io_mux_mic_ctl;
void __iomem *quin_gp_io_mux_qui_ctl;
void __iomem *quin_gp_io_lpaif_qui_pcm_sec_mode_muxsel;
void __iomem *quin_gp_io_mux_qui_ext_clk_ctl;
#endif
...
// 2018-1-4 kh_shin remove mclk
#ifdef MCLK
//Add mclk cfg 
static struct afe_clk_set mclk_1 = { 
...
#if defined(CONFIG_TARGET_PRODUCT_IF_S200N) || defined(CONFIG_TARGET_PRODUCT_IF_S700N) || defined(CONFIG_TARGET_PRODUCT_IF_MKT200)
extern void tas5782_powerdown_request(bool enable);
#endif

#ifndef UNUSED_MSM8X16_WCD
...
static int enable_spk_ext_pa(struct snd_soc_codec *codec, int enable)
{
	...
	if (enable) {
		ret = msm_gpioset_activate(CLIENT_WCD_EXT, "ext_spk_gpio"); //wjhyun TODO : Model define
		if (ret) {
			pr_err("%s: gpio set cannot be de-activated %s\n",
		...
		gpio_set_value_cansleep(pdata->spk_ext_pa_gpio, enable);
		ret = msm_gpioset_suspend(CLIENT_WCD_EXT, "ext_spk_gpio"); //wjhyun TODO : Model define
		if (ret) {
			pr_err("%s: gpio set cannot be de-activated %s\n",
					__func__, "ext_spk_gpio");
			return ret;
		...
		struct msm8916_asoc_mach_data *pdata)
{
	...
	if (pdata->us_euro_gpio < 0) {
		...
		ret = msm_get_gpioset_index(CLIENT_WCD_EXT,
						"us_eu_gpio"); //wjhyun TODO : Model define
		if (ret < 0) {
	...
	if (enable) {
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			if (pdata->afe_clk_ver == AFE_CLK_VERSION_V1) {
				...
				// 2018-1-4 kh_shin remove mclk
				#ifdef MCLK
				mclk_1.enable = enable; 
				ret = afe_set_lpass_clock_v2(port_id,&mclk_1);
				#endif
			}
...
static bool mi2s_started = false;

int set_mi2s_force_enable(void)
{
	int ret = 0;
	...
	mi2s_rx_clk.clk_id = msm8952_get_clk_id(AFE_PORT_ID_PRIMARY_MI2S_RX);
	mi2s_rx_clk.clk_freq_in_hz = get_mi2s_rx_clk_val(AFE_PORT_ID_PRIMARY_MI2S_RX);
	ret = afe_set_lpass_clock_v2(AFE_PORT_ID_PRIMARY_MI2S_RX, &mi2s_rx_clk);
	// 2018-1-4 kh_shin remove mclk
	#ifdef MCLK
	mclk_1.enable = true; 
	ret = afe_set_lpass_clock_v2(AFE_PORT_ID_PRIMARY_MI2S_RX,&mclk_1);
	#endif
	...
	ret = afe_port_start(AFE_PORT_ID_PRIMARY_MI2S_RX, &port_config, 48000);
	if(IS_ERR_VALUE(ret)) {
		pr_info("%s: fail to open AFE port\n", __func__);
		return -EINVAL;
	}
	
	return ret;
}
EXPORT_SYMBOL(set_mi2s_force_enable);


static int msm_q6_enable_mi2s_clocks(struct snd_pcm_substream *substream, bool enable)
{
	int ret = 0;    
    struct snd_soc_pcm_runtime *rtd = substream->private_data;
    struct snd_soc_card *card = rtd->card;
	...
	pr_info("[%s] port id: 0x%x, Stream: %s\n", __func__, port_id, substream->stream?"CAPTURE":"PLAYBACK");
	...
	if (enable) 
	{
		...
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			if (pdata->afe_clk_ver == AFE_CLK_VERSION_V1) {
				pr_info("[%s] AFE_CLK_VERSION_V1\n", __func__);
				mi2s_rx_clk_v1.clk_val1 =
						get_mi2s_rx_clk_val(port_id);
				ret = afe_set_lpass_clock(port_id,
							&mi2s_rx_clk_v1);
			} else {
				pr_info("[%s] AFE_CLK_VERSION_V2\n", __func__);
				mi2s_rx_clk.enable = enable;
				mi2s_rx_clk.clk_id =
						msm8952_get_clk_id(port_id);
				mi2s_rx_clk.clk_freq_in_hz =
    	    ...
    	    port_config.i2s.sample_rate = 48000;
    	    port_config.i2s.ws_src = 1;
        	ret = afe_port_start(port_id, &port_config, 48000);
			if(IS_ERR_VALUE(ret)) {
            	pr_info("%s: fail to open AFE port\n", __func__);
            	return -EINVAL;
        	}
...
bool is_mi2s_enable(void)
{
    return mi2s_started;
}
EXPORT_SYMBOL(is_mi2s_enable);

void set_mi2s_enable(bool enable)
{
    mi2s_started = enable;
}
EXPORT_SYMBOL(set_mi2s_enable);

int set_mi2s_force_disable(void)
{
	int ret = 0;
	...
	ret = afe_close(AFE_PORT_ID_PRIMARY_MI2S_RX);
    if (IS_ERR_VALUE(ret)) {
    	printk(KERN_ERR"fail to close AFE port\n");
    	return -EINVAL;
    }
...
EXPORT_SYMBOL(set_mi2s_force_disable);
#endif /* MI2S_CLOCK_ALWAYS_ON */
...
static int msm_mi2s_snd_startup(struct snd_pcm_substream *substream)
{
	...
	#else
	if(!is_mi2s_enable()) {
		set_mi2s_enable(true);
		msm_q6_enable_mi2s_clocks(substream, true);	
	}
	#endif
...
#if defined(CONFIG_TARGET_PRODUCT_IF_S200N)
	tas5782_powerdown_request(false);		
#endif
...
#if defined(CONFIG_TARGET_PRODUCT_IF_S200N)
	tas5782_powerdown_request(true);
#endif
}
...
static int msm_sec_mi2s_snd_startup(struct snd_pcm_substream *substream)
{
	...
	if ((pdata->ext_pa & SEC_MI2S_ID) == SEC_MI2S_ID) {
		...
		pr_debug("%s(): SEC I2S gpios turned on  = %s\n", __func__,
				"sec_i2s");
		#ifdef GPIO_EXTERNAL_I2S
		ret = msm_gpioset_activate(CLIENT_WCD_EXT, "sec_i2s");
		#else
	...
	if (ret < 0) {
		...
		#ifdef GPIO_EXTERNAL_I2S
		ret = msm_gpioset_suspend(CLIENT_WCD_EXT, "sec_i2s");
		#else
...
static int msm_quat_mi2s_snd_startup(struct snd_pcm_substream *substream)
{
	...
	if (pdata->vaddr_gpio_mux_mic_ctl) {
		val = ioread32(pdata->vaddr_gpio_mux_mic_ctl);
		#ifdef QUAT_MI2S2_SLAVE  
		val = 0x00020000;//joyce note: only set bit 17:16 to 10 to enable quat slave mode
		#else
		val = val | 0x02020002;
		#endif
		iowrite32(val, pdata->vaddr_gpio_mux_mic_ctl);
		...
		#ifdef QUAT_MI2S2_SLAVE 
		val = ioread32(quat_ext_clk_addr); 
		val = val | 0x00000001; 
		iowrite32(val,quat_ext_clk_addr); 
		#endif 
	}
	ret = msm_mi2s_sclk_ctl(substream, true);
	if (ret < 0) {
...
static int msm_quin_mi2s_snd_startup(struct snd_pcm_substream *substream)
{
	...
	#ifdef QUIN_MI2S_SLAVE
    val = ioread32(quin_gp_io_lpaif_qui_pcm_sec_mode_muxsel);
    val = 0x00000000;
    iowrite32(val, quin_gp_io_lpaif_qui_pcm_sec_mode_muxsel);

    val = ioread32(quin_gp_io_mux_qui_ctl);
    val = 0x00000001;
    iowrite32(val, quin_gp_io_mux_qui_ctl);

    val = ioread32(quin_gp_io_mux_qui_ext_clk_ctl);
    val = 0x00000001;
    iowrite32(val, quin_gp_io_mux_qui_ext_clk_ctl);
	#endif /* QUIN_MI2S_SLAVE */
...
#if defined(CONFIG_SND_SOC_TAS2559) || defined(CONFIG_SND_SOC_TAS5782)
static const struct snd_soc_dapm_widget tas5782m_dapm_widgets[] = {
	SND_SOC_DAPM_SUPPLY_S("MCLK", -1, SND_SOC_NOPM, 0, 0,
		msm8952_mclk_event, SND_SOC_DAPM_POST_PMD),
};

static int msm_audrx_init_tas5782m(struct snd_soc_pcm_runtime *rtd)
{
    struct snd_soc_codec *codec = rtd->codec;
	struct snd_soc_dapm_context *dapm = &codec->dapm;
	struct snd_soc_dai *cpu_dai = rtd->cpu_dai;	
    ...
    snd_soc_dapm_new_controls(dapm, tas5782m_dapm_widgets,
			ARRAY_SIZE(tas5782m_dapm_widgets));
...
static struct snd_soc_dai_link msm8952_dai[] = {
...
#if defined(CONFIG_TARGET_PRODUCT_IF_S700N) || defined(CONFIG_TARGET_PRODUCT_IF_MKT200) // wsshim - for BT-SCO
	{/* hw:x,5 */
...
#if defined(CONFIG_TARGET_PRODUCT_IF_S200N) || defined(CONFIG_TARGET_PRODUCT_IF_S700N) || defined(CONFIG_TARGET_PRODUCT_IF_MKT200) /*by dusin.jang at 20180420*/
	{/* hw:x,12 */
		.name = "MSM8x16 Media5",
	...
	/* Backend I2S DAI Links */
	{
		...
		#if defined(CONFIG_SND_SOC_TAS2559)	
		.codec_dai_name = "snd-soc-dummy-dai",
        .codec_name = "snd-soc-dummy",
		#elif defined(CONFIG_SND_SOC_TAS5782)
		.codec_dai_name = "tas5782m-amplifier",
        .codec_name = "tas5782m.2-004a",	
		#else /* CONFIG_SND_SOC_TAS5782 */
		#ifndef UNUSED_MSM8X16_WCD
		.codec_name     = "cajon_codec",
	...
	},
	{
		...
		.cpu_dai_name = "msm-dai-q6-mi2s.3",
		.platform_name = "msm-pcm-routing",
		#if defined(CONFIG_SND_SOC_CX2092X)
		.codec_dai_name = "cx2092x-aif",
		.codec_name = "cx2092x.5-0041",
		#else
		.codec_dai_name = "snd-soc-dummy-dai",
		.codec_name = "snd-soc-dummy",
		#endif
...
static struct snd_soc_card bear_card = {
    /* snd_soc_card_msm8952 */
#if defined(CONFIG_TARGET_PRODUCT_IF_S700N) || defined(CONFIG_TARGET_PRODUCT_IF_MKT200)
    .name       = "msm8952-snd-card-tas5782",
#else	/* CONFIG_TARGET_PRODUCT_IF_S200N */
	.name		= "msm8952-snd-card-ifs200n",
#endif
    .dai_link   = msm8952_dai,
    .num_links  = ARRAY_SIZE(msm8952_dai),
};
...
#ifdef QUAT_MI2S2_SLAVE 	
	quat_ext_clk_addr = ioremap(LPASS_CSR_GP_IO_MUX_MIC_EXT_CLK_CTL , 4); 
	if(quat_ext_clk_addr == NULL) { 
		pr_err("%s WON : ioremap failure for quat ext clock control virt addr\n", 
		__func__); 
		ret = -ENOMEM; 
		goto err; 
	}	
...
#ifdef QUIN_MI2S_SLAVE
    quin_gp_io_mux_mic_ctl = ioremap(LPASS_CSR_GP_IO_MUX_MIC_CTL, 4);
    if(quin_gp_io_mux_mic_ctl == NULL) {
        pr_err("%s WON : ioremap failure for quin gpio mux mic control virt addr\n",
        __func__);
        ret = -ENOMEM;
        goto err;
    }

    quin_gp_io_mux_qui_ctl = ioremap(LPASS_CSR_GP_IO_MUX_QUI_CTL, 4);
    if(quin_gp_io_mux_qui_ctl == NULL) {
        pr_err("%s WON : ioremap failure for quin gpio mux qui control virt addr\n",
        __func__);
        ret = -ENOMEM;
        goto err;
    }

    quin_gp_io_lpaif_qui_pcm_sec_mode_muxsel = ioremap(LPASS_CSR_GP_LPAIF_QUI_PCM_SEC_MODE_MUXSEL, 4);
    if(quin_gp_io_lpaif_qui_pcm_sec_mode_muxsel == NULL) {
        pr_err("%s WON : ioremap failure for quin gpio lpaif qui pcm sec mode muxsel virt addr\n",
        __func__);
        ret = -ENOMEM;
        goto err;
    }

    quin_gp_io_mux_qui_ext_clk_ctl = ioremap(LPASS_CSR_GP_IO_MUX_QUI_EXT_CLK_CTL, 4);
    if(quin_gp_io_mux_qui_ext_clk_ctl == NULL) {
        pr_err("%s WON : ioremap failure for quin gpio mux qui external clock control virt addr\n",
        __func__);
        ret = -ENOMEM;
        goto err;
    }
