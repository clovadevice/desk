				    ...
				    unsigned char value)
{
	...
	if (nResult < 0)
		...
		dev_dbg(pTAS2559->dev, "%s[0x%x] OK\n",
			__func__, addr);
#endif
					...
					unsigned int len)
{
	...
	if (nResult < 0)
		...
		dev_dbg(pTAS2559->dev, "%s[0x%x] OK len: %u\n",
			__func__, addr, len);
#endif
...
#if 0 // hjkoh
		dev_dbg(pTAS2559->dev, "%s[0x%x] %02x OK\n", __func__, addr, (unsigned char)val);
#endif
	}
...
#if 0 // hjkoh
	dev_info(pTAS2559->dev, "tas2559_change_book_page ch: %d\n", (int)chn);
#endif
...
#if 1 // hjkoh
	tas2559_enableIRQ(pTAS2559, DevA, false);
#else
...
#if 1 
static int __tas2559_cdc_mclk_enable(struct tas2559_priv *pTAS2559, bool enable)
{
	int ret = 0;

	dev_dbg(pTAS2559->dev, "WON, enter %s\n", __func__);
...
int tas2559_cdc_mclk_enable(struct snd_soc_codec *codec, int enable, bool dapm)
{
	struct tas2559_priv *pTAS2559 = snd_soc_codec_get_drvdata(codec);

	return __tas2559_cdc_mclk_enable(pTAS2559, enable);
}
EXPORT_SYMBOL(tas2559_cdc_mclk_enable);
#endif
...
static int tas2559_suspend(struct device *dev)
{
...
static int tas2559_resume(struct device *dev)
{
	struct tas2559_priv *pTAS2559 = dev_get_drvdata(dev);
	struct TProgram *pProgram;
	...
	if (pTAS2559->mnCurrentProgram >= pTAS2559->mpCalFirmware->mnPrograms) {
...
#ifdef CONFIG_PM_SLEEP
static const struct dev_pm_ops tas2559_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(tas2559_suspend, tas2559_resume)
};
#endif
